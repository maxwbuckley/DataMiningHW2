...
save(cleaned_data, file = paste0(output_dir, "/",
                                 "cleaned_data.Rda"))
```
With some of my data preprocessing I now had a better idea of what my data source looks like and how it is structured.
...
From this, improved chart we can more clearly see that team performance is pretty volatile, and hence more difficult to predict. Teams can move up and down the leaderboard year to year. We see a lot of the teams at the top of the chart tend to be discontinuous as they get relegated due to poor performance. Though it would probably be easier to digest this data in tabular form. As I show here.
...
Unsurprisingly the half time result is a super strong predictor of the full time result. But it wouldn't be a very good predictive model if we got to use that. As it would imply we were predicting at the half time whistle.
...
Investigate how accurate we can be when predicting with the match stats. Even though that will be unusable in everyday situations. It makes for a nice tree though and a formidable 58.16% accuracy
...
All my clusters are of sufficient size, between 312 and 566 matches. Also we can see visually in our 2 dimensional representation that the clusters are linearly seperable which is a property of centroid based (spatial) clustering. We could draw lines between the different clustered points.

Examing our clusters we find that in cluster 5 we see that 'Away' results are the most common with ~47% of the results which is unusual given 'Away' results are less common in general. Similarly in cluser 4 'Home' victory are much more frequent at ~63%
