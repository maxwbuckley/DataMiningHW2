...
So my trees are more accurate than simple guessing the majority class. 43.88% versus 37.24%. However that is not a huge improvement. Though here I only made use of the teams one both sides and the home/away status.

```{r}
historical_point_record <- calculate_team_perf_pairs("2016-01-01")


training_set<-merge(training_set, historical_point_record, 
                    by=c("TeamName", "Opponent"))

testing_set<-merge(testing_set, historical_point_record, by
                   =c("TeamName", "Opponent"))
...
training_set<-join(training_set, norm_goals, by=c("TeamName",
                                          "Opponent"))
testing_set<-join(testing_set, norm_goals, by=c("TeamName",
                                          "Opponent"))
...
```
Create a metric that looks at how that

```{r}

tree1 <- rpart(Points ~ TeamName + Opponent +HistoricalPointRatio + HistoricalOpponentPointRatio + MeanGoals+ OpponentMeanGoals +WinRate+ LoseRate+ DrawRate, data=training_set)
plot(tree1)
text(tree1)
...
party_tree<-ctree(Points ~ TeamName + Opponent + HistoricalPointRatio + HistoricalOpponentPointRatio + MeanGoals+ OpponentMeanGoals +WinRate+ LoseRate+ DrawRate, data=training_set, controls=ctree_control(maxdepth=2))
plot(party_tree, main="Predicting match results by Decision Tree")
predictions<-fix_preds(party_tree, testing_set)
confusionMatrix(predictions, testing_set$Points)
...
library("gbm")
boost<-gbm(Result~ TeamName + Opponent +
                      HistoricalPointRatio +
                      HistoricalOpponentPointRatio +
                      WinRate+ LoseRate + DrawRate,
                    data=training_set, n.trees=1000)
summary(boost)
boost_pred<-predict(boost, testing_set, n.trees=1000, type="link")

boost_pred_labels <- colnames(boost_pred)[apply(boost_pred, 1, which.max)]

confusionMatrix(boost_pred_labels, testing_set$Result)
